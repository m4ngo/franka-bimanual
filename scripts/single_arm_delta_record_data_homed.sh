...
# The robot runs in EE_DELTA mode: each action from the SpaceMouse is an EE
...
if [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ] || [ -z "$4" ] || [ -z "$5" ] || [ -z "$6" ]; then
    echo "Usage: $0 <repo_id> <num_episodes> <task> <output_dir> <resume> <home_pose_name> [true|false]"
    exit 1
fi
