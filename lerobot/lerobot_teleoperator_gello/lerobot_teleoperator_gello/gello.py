...
class Gello(Teleoperator):
    """
    This is the GELLO teleoperator for the Franka FR3 arm.
    ...
    def __init__(self, config: GelloConfig):
        ...
        self.config = config
        if len(self.JOINT_NAMES) != len(self.config.calibration_position) or len(self.JOINT_NAMES) != len(self.config.joint_signs):
            raise ValueError(
    ...
    def calibrate(self) -> None:
        ...
        calibration = GelloCalibration(
            ...
            gripper_open_position=int(start_joints["gripper"]),
            gripper_closed_position=int(start_joints["gripper"]) - self.config.gripper_travel_counts,
        )
